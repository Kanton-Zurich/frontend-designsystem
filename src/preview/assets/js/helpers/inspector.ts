...
class Inspector extends Helper {
  ...
  constructor() {
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      if (variantsInput.length > 0) {
        variantsInput[variant || 0].checked = true;
        Inspector.triggerVariantChangeOnElement(variantsInput[variant || 0]);
      }
    });
  }
  ...
  public static triggerVariantChangeOnElement(node) {
    const selectorIndex = node.getAttribute('id').replace('variants', '');
    ...
    const disabledColorVariationsElement = document.getElementById(
      `disabledColorVariations${selectorIndex}`
    );
    // update color exceptions
    if (disabledColorVariationsElement) {
      const disabledColors = JSON.parse(
        decodeURIComponent(disabledColorVariationsElement.getAttribute('value'))
      );
      const defaultColor = document
        .getElementById(`defaultColorVariation${selectorIndex}`)
        .getAttribute('value');
      [].slice
        .call(document.querySelectorAll('input[name="colorVariations"]'))
        .forEach((cvInput: HTMLInputElement) => {
    ...
    // add variant to url params
    const url = new URL((window as any).location);
    url.searchParams.set('variant', selectorIndex);
  ...
  public static triggerFilterChangeOnElement(node) {
    ...
    const selector =
      node.type === 'text' ? 'li[data-label-index]' : `li[data-label-index="${node.value}"]`;
